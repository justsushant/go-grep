...
func TestSearchString(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := GrepRun(tc.fs, tc.fileName, bytes.NewReader(tc.stdin), tc.keyword, tc.ignoreCase)
			want := tc.result
...
// 		expErr     error
// 	}{
// 		{name: "greps inside a directory with -r", fs: testFS, stdin: nil, fileName: "", keyword: "test", dirPath: "tests", ignoreCase: false, result: []string{"this is a test file", "one can test a program by running test cases", "this file contains a test line"}, searchDir: true, expErr: nil},
// 	}

// 	for _, tc := range testCases {
// 		t.Run(tc.name, func(t *testing.T) {
// 			got, err := GrepRun(tc.fs, tc.fileName, nil, tc.keyword, tc.ignoreCase, tc.dirPath, tc.searchDir)
// 			want := tc.result
